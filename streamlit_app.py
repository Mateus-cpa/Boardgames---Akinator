...
def similar_caracteristics(df_games, column, wanted_list, title, key_prefix, min_common=1, top_n=5):
...
def init_ludonator_state(df_games, all_characteristics):
    df_total = df_games.copy()
    df_total["ludonator"] = 0
    return {
        "df_total": df_total,
...
def pick_next_question(state):
    ...
    best_score = state["df_total"]["ludonator"].max()
    df_candidates = state["df_total"][state["df_total"]["ludonator"] == best_score]
...
def compute_ludonator_scores(state, answer: str, question: str):
    field, value = question.split(": ", 1)
    value = value.strip()
    ...
    points = points_map.get(answer, 0)
    ...
    state["df_total"].loc[mask, "ludonator"] += points
    state["asked"].append(question)
    state["answers"].append(answer)

    df_results = state["df_total"][["name", "minplayers", "maxplayers", "year", "ludonator"]]
    best_score = df_results["ludonator"].max()
    top_matches = df_results[df_results["ludonator"] == best_score]
    state["results"] = top_matches
    return points_map

def run_ludonator(df_games, all_characteristics):
    if "ludonator_state" not in st.session_state:
        st.session_state["ludonator_state"] = init_ludonator_state(df_games, all_characteristics)

    state = st.session_state["ludonator_state"]
    ...
    if state["step"] == "start":
        players = st.sidebar.number_input("Quantos jogadores o jogo suporta?", min_value=1, max_value=100, value=2)
        
        if st.sidebar.button("Começar!"):
            state["players"] = int(players)
            state["df_total"]["ludonator"] = 0
            state["df_total"]["ludonator"] += state["df_total"].apply(
    ...
    if state["step"] == "question":
        ...
        # pergunta atual
        with st.expander("Pergunta atual", expanded=True):
            st.write(f"**Pergunta {len(state['asked']) + 1}:** {state['current_question']}")
            
            col1, col2, col3 = st.columns(3)
            answer = col1.radio(
                "Sua resposta:",
                ["Sim", "Talvez sim", "Não sei", "Talvez não", "Não"],
                horizontal=True,
                key=f"answer_{state['current_question']}"
            )
            with col2:
                if st.button("Confirmar resposta", key="next_btn"):
                    points_map = compute_ludonator_scores(state, answer, state["current_question"])
        ...
        with st.expander("📊 Debug - Ludonator", expanded=True):
            st.write("### Histórico de respostas")
            st.write(f"Jogadores: {state['players']}")

            df_history = pd.DataFrame({
                "Pergunta": state["asked"],
                "Resposta": state["answers"],
                "Pontos": [points_map.get(a, 0) for a in state["answers"]]
            })
            st.dataframe(df_history, use_container_width=True)

            st.write("### Métricas gerais")
            col1, col2, col3, col4, col5 = st.columns(5)
            col1.metric("Jogos candidatos em 1º lugar", state["results"].shape[0])
            col2.metric("Perguntas respondidas", len(state["asked"]))
            col3.metric("Maior score atual", int(state["df_total"]["ludonator"].max()))
            col4.metric("Menor score atual", int(state["df_total"]["ludonator"].min()))
            col5.metric("Média de score", f"{state['df_total']['ludonator'].mean():.1f}")

            st.write("### Top candidatos")
            st.dataframe(
                state["df_total"][["name", "year", "ludonator"]]
                    .sort_values("ludonator", ascending=False)
                    .head(10),
                use_container_width=True
            )

            st.write("### Distribuição de pontuações")
            score_dist = state["df_total"]["ludonator"].value_counts().sort_index()
    ...
    if state["asked"]:
        ...
        st.write(f"Pontuação mín/máx/média: {state['df_total']['ludonator'].min()} / {state['df_total']['ludonator'].max()} / {state['df_total']['ludonator'].mean():.1f}")

    if state["step"] == "finished":
        if state["results"] is not None and state["results"].shape[0] >= 1:
            best_id = int(state["results"].index[0])
            st.success("O ludonator escolheu um jogo!")
            display_game_info(best_id, df_games)
        else:
            st.warning("Nenhum resultado definitivo foi encontrado.")

def main():
    st.set_page_config(page_title="Boardgame Ludonator", layout="wide")
    st.title("Tela de busca de jogos de tabuleiro")
    st.sidebar.write("Uma interface interativa para encontrar jogos de tabuleiro usando dados do BoardGameGeek.")

    st.session_state.setdefault("chosen_id", None)

    zerar_id = st.sidebar.button("Limpar seleção")
    if zerar_id:
        st.session_state.chosen_id = None
        st.session_state.characteristic_ids = []
        st.session_state.main_menu = "Jogo aleatório"
        st.rerun()
    ...
    st.sidebar.write("## Menu de busca")
    ...
    if page == "Buscar por ID":
        wanted_id = st.sidebar.selectbox("Digite o ID do jogo:", options=df_games.index.tolist())
        #wanted_id = 444
    ...
    # -- MENU 2: NOME PARECIDO --
        ...
        query = st.sidebar.text_input("Digite o nome ou parte do nome do jogo:")
        if query:
            matches = find_similar(query, df_games["name"].tolist(), limit=10)
            chosen = st.sidebar.selectbox("Escolha um jogo", matches)
            if st.sidebar.button("Mostrar detalhes"):
    ...
    # -- MENU 3: JOGO ALEATÓRIO --
        ...
        if st.sidebar.button("Sortear jogo"):
    ...
    # -- MENU 4: POR CARACTERÍSTICA --
        ...
        search_text = st.sidebar.text_input("Buscar característica:")
        ...
        if filtered_characteristics:
            characteristic = st.sidebar.selectbox("Escolha uma característica", [""] + filtered_characteristics)
        else:
            st.info("Nenhuma característica encontrada")
            characteristic = ""
        if st.sidebar.button("Buscar jogos com essa característica"):
    ...
    # -- MENU 5: ludonator --
        ...
        run_ludonator(df_games, all_characteristics)
